...
| 4 | 4 | `ix_events_id` becomes UNIQUE over `(run_id, event_id)`. Analysis bumps in the same commit: it now surfaces the log's own integrity problems (`duplicate_event_id`, `log_integrity`) as findings, which is a change to what it derives. | the evidence-integrity commit |
